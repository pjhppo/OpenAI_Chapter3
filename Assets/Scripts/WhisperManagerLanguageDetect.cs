...
public class WhisperManagerLanguageDetect : MonoBehaviour
{
    ...
    {
        public string text;
        public string language; // 언어 정보를 위한 필드 추가
    }

    public static WhisperManagerLanguageDetect Instance;
    private void Awake()
    {
        if (Instance == null)
        {
    ...
    IEnumerator SendWhisperRequest(string filepath)
    {
        ...
        form.AddField("response_format", "verbose_json"); // 추가된 부분
        ...
        if (request.result == UnityWebRequest.Result.Success)
        {
            ...
            try
            {
                ...
                string detectedLanguage = jsonResponse.language; // 언어 정보 추출

                Debug.Log("인식된 텍스트: " + transcribedText);
                Debug.Log("인식된 언어: " + detectedLanguage); // 언어 정보 로그 출력
